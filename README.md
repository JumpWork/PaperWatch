# PaperWatch
A tool for automatically forwarding CloudWatch logs from your lambda function or ECS containers to Papertrail.
...
| `sources` | Yes | _Array_ of _Objects_ | List of log sources.  A log source is identified by the _prefix_ property.  Log's generated by a source are passed to it's _consumer_ function.  |  
...
```
aws cloudformation create-stack --stack-name paperwatch --template-body file:///<path_to_PaperWatch>/cfn/deploy.cfn.json --parameters ParameterKey=SourceBucket,ParameterValue=jump-lambdas ParameterKey=SourceKey,ParameterValue=lambda.zip --capabilities CAPABILITY_IAM
```
...
3. From the Lambda Function dropdown, select the consumer function (LambdaLogConsumer, ECSLogConsumer or APIGatewayLogConsumer) and click next  
